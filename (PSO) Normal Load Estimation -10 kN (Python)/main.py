...
## You should change the variable nop to adjust number of models
nop  = 5 # Numberof Particles

# (Variable initialization - vectorized where possible)
Ncap    = np.float32(np.linspace(Nmin, Nmax, nop))
Ncap0   = (Nmin + Nmax) / 2
Ncapest = Ncap0
Velp    = np.float32(np.full(nop, (Nmax - Nmin) / (nop - 1)))

costfp    = np.float32(np.zeros(nop))
costgbest = 1e5 # Initially assigned a large value

c1p    = 2.0
omegap = 0.9
tidx   = 1

# Data import from measurements
...
for i in range(ni):
    ...
    if (abs(timeSys - times[tidx]) <= 1e-4) and timeSys!=0:
        ...
        r1p  = np.random.rand(nop)
        Velp = omegap * Velp + c1p * r1p * (Ncapest - Ncap)
        Velp = np.clip(Velp, -1500, 1500)
        if np.mean(np.abs(Velp)) < 10:
...
plt.savefig('Ncap_5Mod_PSO_20kN.pdf', bbox_inches='tight',pad_inches=0.1)  # Save as PDF
plt.show()

# Moving average filter with 1 second of window size
window_size = 2001
if window_size % 2 == 0:
...
plt.savefig('Ncap_Filtrered_5Mod_PSO_20kN.pdf', bbox_inches='tight',pad_inches=0.1)  # Save as PDF
plt.show()
...
# RMSE result for the parameter estimation
